...
- [x] **Audit (Capa 8)**: `Gateway` emite `AddAuditRq` (no `null`) a `SendAsync` — patrón Hotelbeds — con `AuditConfigId`/`SystemUserToken`/`ProviderConnectionId` de `providerParameters` y `AuditType` por operación. _Añadido jun-2026 tras detectarse el gap al probar el registro en local; trasladado al DoD del flujo (`factory_pull_validaciones.md` §Capa 8) para no repetirlo en nuevos conectores._
